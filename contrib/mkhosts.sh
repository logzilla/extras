...
declare -A entries
ips=($(curl -sH "Content-Type: application/json; charset=utf-8" -H "Authorization: token $token" "http://192.168.10.135/api/dictionaries/host?limit=1000" | jq -r '.list[].name' | grep -P '^\d{1,3}\.'))
